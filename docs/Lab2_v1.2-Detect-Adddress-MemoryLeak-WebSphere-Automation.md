<span id="_Toc107577959" class="anchor"></span>**Detect and Address
Application Memory Leak with IBM WebSphere Automation**
...
[**Detect and Address Application Memory Leak with IBM WebSphere
Automation** 1](#_Toc107577959)

[**Introduction to WebSphere Automation** 3](#_Toc107577960)

[**Business Context** 4](#_Toc107577961)

[**Accessing and starting the environment** 6](#_Toc107577962)

[**Part 1: Register WAS/Liberty Servers with WSA**
9](#part-1-register-wasliberty-servers-with-wsa)

[**Part 2: Installing and starting Instana agent**
16](#part-2-installing-and-starting-instana-agent)

[**Part 3: Configure WebSphere Automation integration with Instana and
WebSphere/Liberty Servers**
21](#part-3-configure-websphere-automation-integration-with-instana-and-websphereliberty-servers)

[**Part 4: Configure Instana for memory leak alert**
26](#part-4-configure-instana-for-memory-leak-alert)

[**Part 5: WebSphere memory leak check with WebSphere Automation**
39](#part-5-websphere-memory-leak-check-with-websphere-automation)

[**Part 6: Liberty memory leak check with WebSphere Automation**
48](#part-6-liberty-memory-leak-check-with-websphere-automation)

[**Summary** 52](#_Toc107577969)

[**Appendix 1: Reserve an environment for the lab**
54](#appendix-1-reserve-an-environment-for-the-lab)

[**The lab environment** 57](#the-lab-environment)

<span id="_Toc107577960" class="anchor"></span>**Introduction to
...
<span id="_Toc107577961" class="anchor"></span>**Business Context**
...
Security with IBM WebSphere Automation** documentation. In this lab, you
...
5.  Enter Username and Password as: **ibmadmin**/e**ngageibm** and click
    **Log in**.
...
6.  Open a Terminal window, by clicking **Applications** \>
    **Terminal**.

> ![Graphical user interface, application Description automatically
> generated](./images/media/image17.png)

7.  If you have not cloned the public repo named
...
8.  Run script **register\_was\_liberty** in the
...
> 
> /home/ibmuser/WAS-Automation-LabFiles/lab2-MemoryLeak/register\_was\_liberty.sh

9.  Once the script execution is completed, go back to the WSA page and
...
10. Next, you need to configure an email to received notifications from
    WSA. The Email server configuration is pre-configured for this lab.
    You only need to add your personal email to receive WSA
    notifications.
    
    1.  Select the **Notifications** menu item from the navigation list
...
2.  Click the **Add** button, enter your **email address** and click
    **Save**.
...
> 
> Using the login credentials as:
> 
...
2.  In the Installing Instana Agent page is displayed. Select **Linux**
    OS, **Dynamic** Agent mode, **Eclipse OpenJ9 11** Agent runtime, and
    **Interactive** Installation mode and click **Copy** to copy the
    scrip for installing the Instana agent.
...
5.  Define Instana Infrastructure Zone
    ...
    1.  Open the Instana agent configuration file for editing with the
        command

> sudo gedit /opt/instana/agent/etc/instana/configuration.yaml

2.  Change the Hardware and Zone section as the follows
...
3.  Click **Save** to save the changes, then click **Close** to close
    the editor.

> ![Graphical user interface, text, application Description
> automatically generated](./images/media/image25.png)

6.  Start Instana agent with the command:

> sudo systemctl start instana-agent.service
> 
> If prompted, enter the sudo password as: **engageibm\!**
> 
> Instana agent is now started.

7.  Verify the Instana agent installation

> Note: after the agent is started, it might take a few minutes for the
> agent to sync with Instana server.

1.  Go back to the Instana page in web browser, scroll to the bottom of
    the page and click **Go to Instana\!** to navigate to Instana
    administrative console page.

![Rectangle Description automatically generated with medium
confidence](./images/media/image26.png)

2.  In the Instana administrative console page, click the
    **Infrastructure** icon.

![Graphical user interface, application Description automatically
generated](./images/media/image27.png)

3.  Click Time Rage dropdown button, select **Last 5 minutes** and click
    **Live**.

![A screenshot of a computer Description automatically generated with
medium confidence](./images/media/image28.png)

> After a few minutes, you see that the infrastructure zone you defined
> is displayed.
> 
> ![A picture containing shape Description automatically
> generated](./images/media/image29.png)
> 
> The Istana agent is now set to monitor the changes in 5 minutes
> interval, the agent installation is now completed.
...
> In this part, you create a keystore with the WSA certificate and
> add it to Instana. This is done by using any current keystore (in the
> lab you are using the **cacerts** keystore), import the WSA
> certificate, then copy the keystore to the Instana server and add the
> keystore to the Instana configuration.
> 
...
> 
> When prompted, type **yes** and enter the **ibmadmin** user password
> as: **passw0rd**
...
1.  Access Instana server to adjust Instana settings to use the keystore
    certificate.
...
2.  Edit the **settings.hcl** file.

> sudo vi settings.hcl
> 
> When prompted, enter the **ibmadmin** user password as: **passw0rd**

3.  Add the line **custom\_keystore="/home/ibmadmin/cacerts"** to the
    beginning of the file and save the change.

> ![Graphical user interface, text Description automatically
> generated](./images/media/image30.png)

4.  Update Instana with the command.
...
5.  When the Instana server is updated, verify that the custom keystore
    has been enabled and end the ssh session
...
> In order for WebSphere Automation to be able to collect data for you
> WAS instances, you need to create a SSH key for their communications.
> 
> Note: To save time, this task is already completed for you in this lab
...
1.  Access the WebSphere Automation server to create a new SSH key.

> ssh root@dns
> 
> When prompted, type **yes** and enter the **root** user password as:
> **passw0rd**

2.  Issue command to create a new SSH key with passphrase. In the lab
    the passphrase used is: **passw0rd**
...
3.  Copy the key over to the WAS server to be monitored. In this lab the
...
4.  Test the login to the WAS server from the WebSphere Automation
...
5.  Setting up WebSphere Automation for SSH to Linux or UNIX servers
...
> 
> \--from-literal=ansible\_user=ibmuser \\
> 
> \--from-literal=ansible\_port=22 \\
> 
> \--from-file=ssh\_private\_key\_file=/root/.ssh/wsa \\
> 
> \--from-literal=ssh\_private\_key\_password=passw0rd
> 
> ssh-keyscan student.demo.ibmdte.net \>\> /root/wsa\_known\_hosts
> 
> oc create configmap wsa-ansible-known-hosts
> --from-file=known\_hosts=/root/wsa\_known\_hosts

6.  Test connection

> MANAGER\_POD=$(oc get pod -l
> app.kubernetes.io/component=runbook-manager -o name | head -n 1)
> 
> oc rsh $MANAGER\_POD runcli testConnection student linux
> 
...
7.  Exit the ssh session with command.
...
> Instana administrative console.

1.  From the Instana page, go to **Settings** \> Team Settings page,
    under **Events & Alerts**, click **Custom Payload**.

> ![Graphical user interface, text, application, chat or text message
> Description automatically generated](./images/media/image32.png)

2.  In the **Key** field, type **PID**, do not use any other value for
    the key. Set the **Value type** to **Dynamic**. In
...
2.  Gather the WSA information required to set up the Alert Channel.
    ...
    1.  From the Terminal window, run the following **oc** commands to
...
> 
> oc get route cpd -o
> jsonpath=https://{.spec.host}/websphereauto/health/webhooks/instana/memleak
> 
...
> To create an alert channel by using the Instana administrative
> console,

1.  On the Team Settings page, under **Events & Alerts**, click **Alert
    Channels**.
...
3.  On the Create Generic Webhook Alert Channel page, enter a name of
    your choice in the **Name** field, in this lab the name is:

> WebSphere Automation Healing Webhook

4.  Enter the **Webhook URL** you have for WebSphere
    Automation installation.
...
6.  Create the following two request headers and then click **Test
    Channel**.
...
> 
> you should see the message showing the channel was successfully
> triggered.
> 
...
> To create a customer event by using the Instana administrative
> console,

1.  On the Team Settings page, under **Events & Alerts**,
    click **Events**, then click **New Event**.

> ![Graphical user interface, application Description automatically
> generated](./images/media/image41.png)

2.  Create a custom “**Memory Usage High**” event and set its parameters
    as shown in the screenshot below and click **Create**:
...
2.  In the **Name** field, type a name for the alert as: **WebSphere
    Automation Healing Memory Leak**.

> ![Graphical user interface, text, application, email Description
> automatically generated](./images/media/image44.png)

3.  In the **Events** menu, select **Alert on Event(s)**.

> ![Graphical user interface, application Description automatically
> generated](./images/media/image45.png)

4.  In the **Events** section, click **Add Event**.

> ![Graphical user interface, application Description automatically
> generated](./images/media/image46.png)

5.  In the Add Events window, enter **memory usage h** in the search box
    and select **Memory Usage High**. You can filter the list by using a
...
6.  Add two more events: **Garbage collection activity high** (for JVM)
    and **Memory leak** (for JVM).
...
7.  In the **Scope** menu, select **All available entities**.

> ![Graphical user interface, application, Teams Description
> automatically generated](./images/media/image50.png)

8.  Under **Alerting**, click **Add Alert Channel**, select the alert
    channel that you created previously, and click **Add 1 Channel**.
...
> /opt/IBM/WebSphere/AppServer9056/bin/startServer.sh tWAS\_9056\_server
...
> 
> Since all the application are working fine at this time, you do not
> see anything in the Health page yet.
> 
> ![Graphical user interface, text, application Description
...
3.  In this lab, you can also monitor the WebSphere application server
    ...
    1.  In Instana page, click **Infrastructure** icon, click
        **Comparison Table** select **JVMs** from the dropdown and click
        **bootstrap WAS90.SERV1.**
...
> /home/ibmuser/WAS-Automation-LabFiles/lab2-MemoryLeak/curl\_MLApp\_add\_was.sh
...
6.  Go back to WSA page, you see that WSA has received the Memory leak
    ...
    server name is identified. Click the **Investigating** link to view
    its details.
...
> The screenshot below shows that investigation is in process.,
> 
> ![Graphical user interface, application Description automatically
> generated](./images/media/image63.png)

8.  If you check your email, you should have received a WSA notification
    like the following for the issue.

> ![Graphical user interface, text, application, email Description
> automatically generated](./images/media/image64.png)

9.  In a few minutes, the WSA investigation is completed, you can see
...
![Graphical user interface, text, application Description automatically
generated](./images/media/image65.png)

> ![Graphical user interface, text, application Description
> automatically generated](./images/media/image66.png)
> 
> ![Graphical user interface, application Description automatically
> generated](./images/media/image67.png)
> 
> ![Graphical user interface, text, application, email Description
> automatically generated](./images/media/image68.png)
...
> /home/ibmuser/WAS-Automation-LabFiles/lab2-MemoryLeak/curl\_MLApp\_removeLast.sh
> 
> /opt/IBM/WebSphere/AppServer9056/bin/stopServer.sh tWAS\_9056\_server
...
> /opt/IBM/WebSphere/Liberty200012/bin/server start
> Liberty\_200012\_server
> 
...
2.  Go back to Instana Infrastructure page, select **Comparison Table**
    \> J**VMs** and click the **Liberty\_200012\_server** link.

![Graphical user interface, application Description automatically
generated](./images/media/image70.png)

> You see the JVM dashboard displayed with No Issues status.

![Graphical user interface, application Description automatically
generated](./images/media/image71.png)

3.  From the Terminal window, run the script to increase application
    heap usage which will create application memory leak event.

/home/ibmuser/WAS-Automation-LabFiles/lab2-MemoryLeak/curl\_MLApp\_add\_liberty.sh
...
![Graphical user interface, application Description automatically
generated](./images/media/image72.png)

5.  In the WSA Health page, you see another investigation is started for
    the Liberty server memory leak issue.

![Graphical user interface, application Description automatically
generated](./images/media/image73.png)

> After a few minutes, the investigation is completed.

![Graphical user interface, text, application Description automatically
generated](./images/media/image74.png)

> Since you are using the same application in both WAS and Liberty, you
> should see the similar WSA Liberty memory leak analysis results as
> WAS.
> 
> ![Graphical user interface, text, application, email Description
> automatically generated](./images/media/image75.png)

<span id="_Toc107577969" class="anchor"></span>**Summary**
...
1.  Use your **IBM ID** to login to the IBM Technology Zone

2.  The **Create a reservation** page is displayed
...
2.  Select “**Reserve for Now**” Radio button, and then follow the
    ...
    1.  The environment **name** should be pre-filled with “**WebSphere
        Automation Environment – Skytap**”
    
    2.  **Purpose**: Practice / Self-Education
    
    3.  **Description**: Enter a description. The field is required.
    
    4.  **End date and time**: Use the calendar widget and select the
        maximum date available (2 weeks from date of reservation)
    
    5.  **Select a time**: Select a time of day for reservation to
        expire
    
    6.  Select a **timezone** nearest to you
